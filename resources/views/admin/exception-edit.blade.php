...
<div class="container">
    ...
    <div class="row">
        <div class="col-12">
            <h4 class="mb-3">{{ __('Exception edition') }}</h4>
        </div>
    </div>
        ...
        <div class="col-12">
            <form method="POST" action="{{ route('exception.update', ['id' => $aException['id']]) }}" id="exceptionEditForm">
                ...
                <div class="form-group">
                    ...
                    <div class="form-check">
                        <input id="type_holiday" type="radio" class="form-check-input" name="type" value="holiday"
                            {{ (old('type', $aException['type']) == 'holiday') ? 'checked' : '' }}
                        />
                    ...
                    <div class="form-check">
                        <input id="type_other" type="radio" class="form-check-input" name="type" value="other"
                            {{ (old('type', $aException['type']) == 'other') ? 'checked' : '' }}
                        />
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="observations" name="observations" rows="5">{{ old('observations', $aException['observations']) }}</textarea>
                </div>
...
jQuery(document).ready(function() {
    ...
    function setDateTime(psType) {
        ...
        if ('{{ old('datetimes', $aException['datetimes']) }}' != '') {
            ...
            var aDateTimes = '{{ old('datetimes', $aException['datetimes']) }}'.split('{{ __(' - ') }}');
